...
- Live docker-compose run (`docker compose -f docker-compose.airflow.yml up`,
  trigger `dbt_transform`): both `dbt_run` and `dbt_test` green — see PR
  (executed after the upstream `stg_trips` merge from faza-6-task-1).
